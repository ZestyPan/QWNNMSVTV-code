function [u,g1,g2] = Cauchy_deblurring(x_n,kernel,alpha,M,r,beta,tol,maxiter,gamma,mu)
%% Description
%Solve the model£º
%argmin(u,g1,g2)=1/2£¨log(gamma^2+(Ku-x_n)^2)+mu*||Ku-u_0||.^2£©+alpha*(||g1-M/alpha||.^2+||g2-M/alpha||.^2)+g1.^2.*K(uTV)+g2.^2.*K(uLLT);
...
while i < maxiter.out && crit > tol
    %% 
    [u,p,q,lamta]=ALM_nested(x_n,g,u,g1,g2,psf,beta,p,q,lamta,lamta3,tol,maxiter.in,gamma,mu,u0,kernel);
    %% 
...
%% 
function [u,p,q,lamta]=ALM_nested(x_n,g,u,g1,g2,psf,beta,p,q,lamta,lamta3,tol,maxiter,gamma,mu,u0,kernel)
%% Description
%Solve the model£ºargmin(u)=1/2£¨log(gamma^2+(Ku-x_n)^2)+mu*||Ku-u_0||.^2£©+g1.^2.*K(uTV)+g2.^2.*K(uLLT)
%% 
kkmax = 10;
kk=0;
...
while i < maxiter && crit > tol
    ...
    nomin=-div(lamta.x+beta*p.x,lamta.y+beta*p.y,'circular')+...
        ...
        beta*q.yx,lamta.yy+beta*q.yy,'circular');
    nomin=fft2(nomin/beta)+conj(psf2otf(kernel,[m,n])).*(fft2(g)+fft2(lamta3)*(1/beta));
    ...
    KKu=imfilter(u,kernel,'circular','conv');
    temp=gamma^2+(g-x_n).^2;
    fd =(g-x_n)./temp+mu*(g-u0)+beta*(g-KKu)+lamta3;
