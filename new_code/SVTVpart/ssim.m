...
ssim_map = imfilter(img1,window,'symmetric','same');
ssim_map = real(ifft2(psf2otf(window, size(img1)).*fft2(img1)));

%w1 = filter2(window, img2, 'valid');              % gy
w1 = imfilter(img2,window,'symmetric','same');
w1 = real(ifft2(psf2otf(window, size(img1)).*fft2(img2)));
...
ssim_map = imfilter(img1.*img2,window,'symmetric','same');
ssim_map = real(ifft2(psf2otf(window, size(img1)).*fft2(img1.*img2)));
...
if (C1 > 0 && C2 > 0)
   ...
   w2 = imfilter(img1,window,'symmetric','same');
   w2 = real(ifft2(psf2otf(window, size(img1)).*fft2(img1)));
   w2 = w2-w1+(C1+C2);                            % den2
   w2 = w2.*w1;                                   % den
